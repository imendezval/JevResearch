...
`cifar-run` accepts `--proposal-strategy global-random|global-pool|tpe|cmaes` and requires `--proposal-domain`. `global-random`, `tpe`, and `cmaes` use the single-candidate selector; `global-pool` lets `--controller random|jev` select from up to eight reproducible proposals. Example bounded wiring run:
...
Predeclared multiseed study recipes are [mixed](docs/examples/phase5-mixed.json) and [numeric](docs/examples/phase5-numeric.json). Pass the real cache with `--data-dir /path/to/cache`; use a fresh `--output-root` for each source revision or study specification. The mixed recipe includes global random, TPE, random/global-pool, and Jev/global-pool. The numeric recipe compares global random, TPE, and CMA-ES only within the numeric domain. These runs require an explicit compute budget and are not launched as part of installation. Reports include per-seed trajectories, median/min/max best validation accuracy, failures, timing, and estimated resource cost when a rate is supplied.
